...
/** Customer: virtual-account bank details, renewal, countdown, and soon banner (Proceed to payment). */
...
import type { VirtualAccountBankStepUiState } from "@/app/(customer)/_utils/virtualAccountBankStepUi";

export type VirtualAccountBankPaymentSectionProps = {
  ...
  /** When set, Account Number uses this (e.g. customer toast on copy); otherwise clipboard only. */
    ...
    <>
      ...
      {activeVa ? (
        <div className="flex flex-wrap items-baseline justify-center gap-x-2 gap-y-1 text-center text-sm text-[#8F8B8B] sm:text-base">
          ...
          <span className="text-lg font-bold leading-none text-[#323131] sm:text-xl md:text-2xl">
...
export function VirtualAccountBankPaymentSection({
  ui,
  accountNumber,
  bankName,
  accountName,
  ...
  onCopyAccountNumber,
}: Readonly<VirtualAccountBankPaymentSectionProps>) {
